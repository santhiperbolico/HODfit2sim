...
#    Get automatically the width of bins dependind of the number of values
#
#    Parameters
#    -----------
#    vals : numpy array of floats
#        Array with values to make a histogram with
#    factor : float
#        Factor for the Freedman–Diaconis rule
# 
#    Returns
#    -----
#    binwidth : float
...
#    Examples
#    ---------
#    >> from h2s_stats import get_binwidth
#    >> get_binwidth(np.asarray([0.,1.2,0.5,6.,3.1,0.25]))
#    """
#
#    """
#    Examples
...
def get_binFD(vals,factor=2):
    """
    Get the width of bins following Freedman–Diaconis rule
    ...
    vals : numpy array of floats
        Array with values to make a histogram with
    factor : float
        Set to 2.59 for normal distributions
    ...
    Examples
    ---------
    >> from h2s_stats import get_binFD
    >> get_binFD(np.asarray([0.,1.2,0.5,6.,3.1,0.25]))
    """

    iqr = np.quantile(vals,0.75) - np.quantile(vals,0.25)
    
    nvals = len(vals)    
    binwidth = factor*iqr/pow(nvals,1/3)
    
    return binwidth   


def get_binwidth(vals,factor=2):
    """
    Get automatically the width of bins dependind of the number of values
    ...
    if (nvals < 30):
        binwidth = (np.max(vals)-np.min(vals))/np.sqrt(nvals)
    elif (nvals < 200):
        # Sturge's rule
        binwidth = (np.max(vals)-np.min(vals))/(1 + np.log2(nvals))
    else:
        # Freedman–Diaconis rule
        binwidth = get_binFD(vals,factor=factor)
        
    return binwidth   


if __name__== "__main__":
    vals = np.asarray([0.25,1.2,0.5,6.,3.1,0.])
    print(get_binwidth(vals))
    print(get_binFD(vals))
