...
dirobsari = '/users/'+usern+'/trunk/Obs_Data/'
dirplotari = '/users/'+usern+'/output/'
...
def mb2lmsun(massb,verbose=False):
    """
    Provide bahamas masses as log10(M/Msun/h)

    Parameters
    -----------
    massb : array of floats
    ...
    Returns
    -----
    lmass : array of float
        log10(M/Msun/h)

    Examples
    ---------
    >>> import h2s_bahamas as b
    >>> b.mb2lmsun(np.array([0.0048,1.]))
    """
...
def get_particle_files(snap,sim,env,subfind=True):
    ...
    ---------
    >>> import h2s_bahamas as b
    >>> b.get_particle_files(27,'L400N1024/WMAP9/Sims/BAHAMAS','arilega')
    """
...
def get_resolution(sim,env,snap,dirz=None,verbose=False):
    ...
    -----
    mdm: float
        Mass resolution log10(Msun/h) for DM particles

    Examples
    ---------
    >>> import h2s_bahamas as b
    >>> b.get_resolution('HIRES/AGN_TUNED_nu0_L050N256_WMAP9','arilega')
    """
    ...
    f= h5py.File(files[0],'r') #;print(files[0]);exit()
    header=f['Header']
    ...
    if (mdm<0):
        ...
        if (verbose):
            print('DM particle res., log10(Msun/h) = {:.2f} \n'.format(mdm))
        return mdm
...
#    >>> b.get_nh(0.5,'FOF/Group_M_Mean200',sim,'arilega',outdir='/hpcdata4/'+usern+'/BAHAMAS/')
#    '''
#
...
#    >>> import bahamas as b
#    >>> sim = 'HIRES/AGN_TUNED_nu0_L050N256_WMAP9'
#    >>> b.get_propfunc(31,['FOF/Group_M_Mean200'],'mass',sim,'arilega',outdir='/hpcdata4/'+usern+'/Junk/')
#    '''
#
...
#                         dirz='/users/'+usern+'/output/BAHAMAS/',rewrite=True,
#                         outdir='/users/'+usern+'/output/Junk/',Testing=True)
#    """
#
#    # Output file
